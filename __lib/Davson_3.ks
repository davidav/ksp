//Davson_3  миссия по первому выходу на орбиту Кербина и посадка второй ступени
// 0. вкл двигатель в ручную. затем запуск прграммы (для миссии исп дв на орбите)
// 1. выход на орбиту Кербина (81км)
// 2. облет Кербина 930c
// 3. первый тормозной маневр до апоапсиса 37000
// далее ручками
// 4. второй тормозной маневр в районе КСЦ
// 5. раскрытие парашута 1500м 



set nameProgram to "Davson_3".

set keyreleseSolidBoosters to TRUE. // сброс твердотельных ускорителей

set namePartSideEngines to "NONE".
set keyRealeseSideTanks to FALSE.

set altReleasePebkac to 37000.//высота сброса Pebkac
set keyRealesePebkac to FALSE.
...
until cyclogram = 0 {
    if cyclogram = 1{
        ...
        system["timer"](1, "BEFORE LAUNCH ", 1).
        set cyclogram to 2.
    }  
    if cyclogram = 2{

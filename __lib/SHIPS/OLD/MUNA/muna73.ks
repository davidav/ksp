...
function emptytank{
  ...
  FOR eng IN myEngines {
    if eng:TAG = TankName {
        if eng:FLAMEOUT {
        STAGE.
        print "Отделение пустых баков" at(5,10).
        }
    }
  }
}
...
FUNCTION ApoBurn
{
	...
	set Fii to arcsin(DeltaA/AThr). // Считаем угол к горизонту так, чтобы держать вертикальную скорость = 0.
    set Fi to max(1, Fii).
...
until cyclogram = 0 {
    if cyclogram = 1{
        ...
        print "CYCLOGRAM: "+cyclogram+" - "+namecyclogram at(5,2).
        set GTstart to 10000.// высота начала разворота
        ...
        set GTendAP to 100000. // заканчиваем разворот, когда апоцентр на этой высоте
